...
namespace GrpcService.Services
{
    ...
    {
        public override async Task GetSalesData(Request request, IServerStreamWriter<SalesDataModel> responseStream, ServerCallContext context)
        {
            using (var reader = new StreamReader(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Data", @"C:\Users\Keith\Downloads\5m-Sales-Records\5m Sales Records.csv")))
            {
                string line;
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    try
                    {
                        ...
                        _model.ShipDate = Google.Protobuf.WellKnownTypes.Timestamp.FromDateTime((DateTime.TryParse(pieces[7], out DateTime _dateShip) ? _dateShip : DateTime.MinValue).ToUniversalTime());
